...
func (e *engine) dynamicSelfEnergy3() []float64 {
	...
	// The (p,q) pairs are the work item: nvir·nocc = 154·58 = 8932 of them at production scale, and each
	// one is enormous — (C.12)-(C.23) run i·a·b·(c·d + j·k) ≈ 1.4e6·3.3e4 and five more terms of
	// the same shape, so the whole routine is 1e10-1e11 integral evaluations. It is the largest
	// raw cost in this package. It is NOT on the production Σ(∞) path (Static skips straight to
	// densityAllOrder there) — only Four/FourPlus and Density(order=3) reach it — but on those it
	// was the entire runtime, on one core.
	//
	// Pair (p,q) writes the single cell mak[(p−nocc)·nocc+q] and no other; p is virtual, q is
	// occupied and both come from the same irrep, so every pair is enumerated exactly once. The
	// twelve private accumulators c1..c6/d1..d6 and their summation order are untouched, so the
	// value written is bit-identical to the serial walk.
	//
	// HeavyRows, not Rows: nvir·nocc collapses with point-group symmetry and one item is millions
	// of flops, exactly the case Rows' "< 2·GOMAXPROCS rows" fallback gets backwards.
	var pairs [][2]int
