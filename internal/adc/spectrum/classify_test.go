...
// h2oGroups mirrors the population grouping of the H2O test data, with each atom
// column as its own site (the identity grouping).
var h2oGroups = []Site{
	{Name: "O"}, {Name: "H1"}, {Name: "H2"},
}

// firstStateRow is the population row of the 39.6604 eV state in the DIP h2o
// reference (adcdip1.out):
//
//	O      O/H1    O/H2    H1      H1/H2   H2
//	0.8065 0.0131  0.0131  0.0004  0.0002  0.0004
func firstStateRow() Row {
	return Row{
		EnergyEV: 39.6604,
...
func weightOf(chs []Channel, name string) (float64, bool) {
	for _, c := range chs {
		if c.Name == name {
...
// The sum of all channel weights must equal the row's total two-hole population:
// classification routes, it never creates or destroys weight.
...
// holes can sit, so the sole possible channel is local Auger.
func TestClassifySingleSiteOnlyAuger(t *testing.T) {
	water := []Site{{Name: "water", Members: []string{"O", "H1", "H2"}}}
...
// straddling an active and a passive column, and removes an all-passive pair.
func TestDiscountHalvesPassivePairs(t *testing.T) {
	passive := map[string]bool{"H1": true, "H2": true}
	got := Discount(firstStateRow(), passive)

	wantOne := map[string]float64{"O": 0.8065}
	wantTwo := map[string]float64{"O/H1": 0.0131 / 2, "O/H2": 0.0131 / 2}
...
func TestDiscountNoPassiveIsIdentity(t *testing.T) {
	row := firstStateRow()
	got := Discount(row, PassiveSet([]Site{{Name: "O"}, {Name: "H1"}, {Name: "H2"}}))
